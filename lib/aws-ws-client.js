class AwsWebSocketClientWrapper{
    ...
    subscribe(channel){
        this.socket.send(JSON.stringify({action: "subscribe", data: channel}));
    }

    unsubscribe(channel){
        this.socket.send(JSON.stringify({action: "unsubscribe", data: channel}));
    }
    ...
    websocketInit () {
        ...
        const onClose = (e) => {
            if(this.debug)
            console.log(`Socket Connection Closed:- [State: ${this.socket.readyState}, closedPermanently: ${this.socketIsClose}]`);
            if(!this.socketIsClose) // CLosed automatically
            this.websocketReconnect() // Try reconnecting
        }
...
export default AwsWebSocketClientWrapper;